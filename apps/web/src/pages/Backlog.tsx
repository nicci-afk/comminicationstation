...
export default function Backlog() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      ...
      {isLoading && <div className="mt-6 text-slate-400 dark:text-slate-500">Loading…</div>}
      ...
      <div className="mt-4 space-y-3">
        {groups.map((g) => (
          <div key={g.sender} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
            <div className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <div className="font-medium truncate">{g.name}</div>
                <div className="text-xs text-slate-400 dark:text-slate-500 truncate">{g.sender} · {g.items.length} message{g.items.length === 1 ? "" : "s"}</div>
              </div>
              <button
                onClick={() => sweep.mutate({ sender: g.sender, mute: false })}
                className="text-xs bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg px-3 py-1.5"
              >
                Clear all
              </button>
              <button
                onClick={() => sweep.mutate({ sender: g.sender, mute: true })}
                className="text-xs bg-red-50 dark:bg-red-950 hover:bg-red-100 dark:hover:bg-red-900 text-red-700 dark:text-red-400 rounded-lg px-3 py-1.5"
              >
                Clear & mute sender
              </button>
            </div>
            <ul className="mt-2 space-y-1">
              {g.items.slice(0, 3).map((i) => (
                <li key={i.id} className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <Link to={`/item/${i.id}`} className="truncate hover:text-indigo-600 dark:hover:text-indigo-400 flex-1">{i.title || "(no subject)"}</Link>
                  <button onClick={() => promote.mutate(i.id)} className="text-indigo-600 dark:text-indigo-400 hover:underline shrink-0">
                    needs attention →
                  </button>
                </li>
              ))}
              {g.items.length > 3 && <li className="text-xs text-slate-300 dark:text-slate-600">+ {g.items.length - 3} more</li>}
            </ul>
          </div>
        ))}
      </div>
