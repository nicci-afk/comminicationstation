  ...
  return (
    <>
      ...
      <Card title="Gmail accounts" subtitle="Connect every inbox you want in the queue. Read-only access — you keep replying from Gmail.">
        ...
        {deepBackfillMsg && (
          <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">{deepBackfillMsg}</p>
        )}
      </Card>
    ...
    <div className="mt-2">
      ...
      <div className="flex gap-2 mt-1">
        <input
          ...
          className="flex-1 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-1.5 text-sm font-mono bg-white dark:bg-slate-800 dark:text-slate-100"
        />
        {secret && (
          <button
            type="button"
            onClick={() => setShow((s) => !s)}
            className="text-sm text-slate-500 dark:text-slate-400 px-2"
  ...
  return (
    <div>
      <div className="flex items-baseline gap-2">
        ...
        <span className={`ml-auto text-xs ${status === "not set" ? "text-slate-400 dark:text-slate-500" : "text-emerald-600 dark:text-emerald-400"}`}>{status}</span>
      </div>
      <div className="flex gap-2 mt-1">
      ...
      <Card title="Your businesses" subtitle="Messages get filed into these. Your account only — your husband sets up his own.">
        <ul className="space-y-1">
          {businesses.map((b) => (
            <li key={b.id} className="flex items-center gap-2 text-sm">
              ...
              {b.name} {b.is_default && <span className="text-xs text-slate-400 dark:text-slate-500">(default bucket)</span>}
            </li>
          ))}
        </ul>
        <div className="flex gap-2 mt-3">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Business name"
            className="flex-1 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-1.5 text-sm bg-white dark:bg-slate-800 dark:text-slate-100" />
          <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="w-10 h-9 border border-slate-300 dark:border-slate-600 rounded-lg" />
        ...
        <ul className="mt-3 space-y-1 text-sm">
          {rules.map((r: Record<string, unknown>) => (
            <li key={String(r.id)} className="flex items-center gap-2">
              <span className="text-xs bg-slate-100 dark:bg-slate-800 rounded px-1.5 py-0.5">{String(r.rule_type).replace("_", " ")}</span>
...
function RuleForm({ businesses, onAdd }: { businesses: { id: string; name: string }[]; onAdd: (r: Record<string, unknown>) => void }) {
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-6 gap-2 text-sm">
      ...
      <select value={category} onChange={(e) => setCategory(e.target.value)} className="border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-800 dark:text-slate-100">
        <option value="">category…</option>
        {["needs_reply", "fyi", "promotion", "expense", "receipt", "notification", "newsletter", "scheduling", "urgent", "other"].map((c) => (
          <option key={c} value={c}>{c}</option>
        ))}
      </select>
      <select value={action} onChange={(e) => setAction(e.target.value)} className="border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-800 dark:text-slate-100">
  ...
  return (
    <Card title="AI spend this month" subtitle="Every model call is metered. Hard caps stop spend — nothing queues up.">
      <div className="text-3xl font-bold">${total.toFixed(2)}
        <span className="text-sm font-normal text-slate-400 dark:text-slate-500"> / ${Number(data?.caps?.monthly_cap_usd ?? 25)} cap</span>
      </div>
      <ul className="mt-3 space-y-1 text-sm">
        {[...byPurpose.entries()].sort((a, b) => b[1] - a[1]).map(([p, c]) => (
          <li key={p} className="flex justify-between">
            <span className="text-slate-600 dark:text-slate-400">{p.replaceAll("_", " ")}</span>
            <span className="font-mono">${c.toFixed(3)}</span>
          </li>
        ))}
        {byPurpose.size === 0 && <li className="text-slate-400 dark:text-slate-500">No AI spend yet this month.</li>}
      </ul>
      ...
      <Card title="Morning digest" subtitle="One email a day with what needs you — not constant pings. Timezone-aware for travel.">
        <div className="flex items-center gap-3 text-sm">
          ...
          <select value={profile.digest_hour} onChange={(e) => save.mutate({ digest_hour: Number(e.target.value) })}
            className="border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-800 dark:text-slate-100">
          ...
          <input
            ...
            className="border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 flex-1 bg-white dark:bg-slate-800 dark:text-slate-100"
      ...
      <div className="space-y-3 max-w-lg text-sm">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 bg-white dark:bg-slate-800 dark:text-slate-100"
          />
          <button
          ...
          <span className={`text-xs ${cfg?.resend_api_key ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
      ...
      {data && (
        <>
          <table className="w-full text-sm">
            <tbody>
              {data.queues.map((q) => (
                <tr key={q.queue} className="border-b border-slate-100 dark:border-slate-800">
                  <td className="py-1.5">{q.queue}</td>
                  <td className={`text-right font-mono ${q.length > 20 ? "text-amber-600" : ""}`}>{q.length} queued</td>
                  <td className={`text-right font-mono text-xs ${q.oldest_msg_age_sec > 600 ? "text-red-600" : "text-slate-400 dark:text-slate-500"}`}>
          ...
          <p className={`mt-2 text-sm ${data.dead_letters_7d > 0 ? "text-red-600 dark:text-red-400" : "text-slate-500 dark:text-slate-400"}`}>
