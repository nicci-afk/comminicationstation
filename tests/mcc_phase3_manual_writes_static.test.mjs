...
const types = fs.readFileSync("apps/web/src/lib/types.ts", "utf8");
...
assert.ok(sql.includes("grant update (state,execution_owner,blocked_reason,completed_at)"));
...
assert.ok(sql.includes("Unsupported MCC manual action"));
assert.ok(sql.includes("v_is_undo"));
assert.ok(sql.includes("BLOCKED requires blocked_reason"));
assert.ok(sql.includes("DONE requires completed_at"));
assert.ok(sql.includes("revoke insert, update, delete on public.obligation_events"));
assert.ok(sql.includes("revoke insert, update, delete on public.obligation_sources"));
...
assert.ok(executive.includes("Save blocker"));
assert.ok(executive.includes("Undo"));
assert.ok(executive.includes("Show evidence & history"));
assert.ok(executive.includes("Resolve the authoritative source first"));

assert.ok(hooks.includes("useObligationAction"));
assert.ok(hooks.includes("useObligationEvents"));
assert.ok(hooks.includes('action: { type: "UNDO" }') === false);
assert.ok(hooks.includes('"MANUAL_UPDATE"'));
assert.ok(hooks.includes('"mcc-executive-ui"'));
assert.ok(hooks.includes('.eq("user_id", userId)'));
assert.ok(types.includes("export interface ObligationEvent"));
assert.ok(policy.includes('executiveUiMode: "MANUAL_WRITES_PREVIEW"'));

console.log("PASS: MCC Phase 3 manual writes remain least-privilege, audited, reversible, and preview-only");
